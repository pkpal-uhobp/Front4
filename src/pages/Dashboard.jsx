...
const Dashboard = () => {
  ...
  const inProgressItems = technologies.filter(t => t.status === STATUS.IN_PROGRESS);

  // Get completed items
  const completedItems = technologies.filter(t => t.status === STATUS.COMPLETED);
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <Card sx={{ height: '100%', borderTop: 3, borderColor: 'grey.400' }}>
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
              ...
              <List>
                {inProgressItems.map((item, index) => (
                  <Box key={item.id}>
                    ...
                    <ListItem
                      component="div"
                      onClick={() => navigate(`/technology/${item.id}`)}
                ...
                {upcomingDeadlines.map((item, index) => {
                  ...
                  return (
                    <Box key={item.id}>
                      ...
                      >
                        <ListItemIcon>
                          <EventIcon color={isOverdue ? 'error' : 'action'} />
                        </ListItemIcon>
                        <ListItemText
                          primary={item.title}
                          secondary={`Дедлайн: ${new Date(item.deadline).toLocaleDateString('ru-RU')}`}
                        />
                        <Chip
                          label={isOverdue ? 'Просрочено' : STATUS_LABELS[item.status]}
                          size="small"
